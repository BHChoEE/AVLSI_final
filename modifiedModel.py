...
def MobileNet_res(input_tensor=None, input_shape=None, alpha=1, shallow=False, classes=1000):
    ...
    x = DepthwiseConvolution2D(int(64 * alpha), (3, 3), strides=(1, 1), padding='same', use_bias=False)(xr)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Convolution2D(int(64 * alpha), (1, 1), strides=(1, 1), padding='same', use_bias=False)(x)
    x = Add()([x, xr])
    ...
    x = DepthwiseConvolution2D(int(128 * alpha), (3, 3), strides=(1, 1), padding='same', use_bias=False)(xr)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Convolution2D(int(128 * alpha), (1, 1), strides=(1, 1), padding='same', use_bias=False)(x)
    x = Add()([x, xr])
    ...
    model = Model(inputs, out, name='mobilenet_res')
    model.summary()
    return model

def normalCNN(input_tensor=None, input_shape=None, alpha=1, shallow=False, classes=1000):
    from keras.applications.imagenet_utils import _obtain_input_shape
    from keras import backend as K
    ...
    x = Convolution2D(int(64 * alpha), (3, 3), strides=(2, 2), padding='same', use_bias=False)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)

    x = Convolution2D(int(128 * alpha), (3, 3), strides=(2, 2), padding='same', use_bias=False)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    ...
    # x = Convolution2D(int(256 * alpha), (3, 3), strides=(2, 2), padding='same', use_bias=False)(img_input)
    # x = BatchNormalization()(x)
    # x = Activation('relu')(x)
    ...
    model = Model(inputs, out, name='normalCNN')
    model.summary()
    return model


def MobileNet_dis(input_tensor=None, input_shape=None, alpha=1, shallow=False, classes=1000):
    from keras.applications.imagenet_utils import _obtain_input_shape
    from keras import backend as K
    from keras.layers import Input, Convolution2D, Add, \
        GlobalAveragePooling2D, Dense, BatchNormalization, Activation
...
def AlexNet(input_tensor=None, input_shape=None, alpha=1, shallow=False, classes=1000):
    from keras.applications.imagenet_utils import _obtain_input_shape
    from keras import backend as K
    from keras.layers import Input, Convolution2D, Flatten, \
        GlobalAveragePooling2D, Dense, BatchNormalization, Activation, MaxPooling2D
    from keras.models import Model
    from keras.engine.topology import get_source_inputs
    from depthwise_conv2d import DepthwiseConvolution2D
    ...
    x = Convolution2D(int(64 * alpha), (3, 3), strides=(1, 1), padding='same', use_bias=False)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding='valid')(x)

    x = Convolution2D(int(128 * alpha), (3, 3), strides=(1, 1), padding='same', use_bias=False)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    ...
    model = Model(img_input, out, name='alexnet2')
    model.summary()
    return model