...
def preprocessing(filePath):
    ...
    for i in range(len(train_data)) :
        train_data[i] -= np.average(train_data[i])
...
def MobileNet(input_tensor=None, input_shape=None, alpha=1, shallow=False, classes=1000):
    ...
    from keras.layers import Input, Convolution2D, \
        GlobalAveragePooling2D, Dense, BatchNormalization, Activation, Add
    ...
    x = DepthwiseConvolution2D(int(256 * alpha), (3, 3), strides=(1, 1), padding='same', use_bias=False)(xr)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Convolution2D(int(256 * alpha), (1, 1), strides=(1, 1), padding='same', use_bias=False)(x)
    x = Add()([x, xr])
    ...
    xr = Activation('relu')(x)
        ...
        #for _ in range(5):
#---------------------------------
    x = DepthwiseConvolution2D(int(512 * alpha), (3, 3), strides=(1, 1), padding='same', use_bias=False)(xr)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Convolution2D(int(512 * alpha), (1, 1), strides=(1, 1), padding='same', use_bias=False)(x)
    x = Add()([x, xr])
...
#---------------------------------
    ...
    x = DepthwiseConvolution2D(int(1024 * alpha), (3, 3), strides=(1, 1), padding='same', use_bias=False)(xr)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Convolution2D(int(1024 * alpha), (1, 1), strides=(1, 1), padding='same', use_bias=False)(x)
    x = Add()([x, xr])
...
def AlexNet(input_tensor=None, input_shape=None, alpha=1, shallow=False, classes=1000):
    ...
    from keras import backend as K
    from keras.layers import Input, Convolution2D, \
        GlobalAveragePooling2D, Dense, BatchNormalization, Activation, Add,MaxPooling2D
    from keras.models import Model
    from keras.engine.topology import get_source_inputs
    from depthwise_conv2d import DepthwiseConvolution2D
...
def main():
    ...
    ### model   
    model = AlexNet(input_shape = (64, 64, 3), shallow = True, classes = 200)
    plot_model(model,to_file='zz.png')
    ### training
